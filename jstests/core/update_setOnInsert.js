...
if (!res.hasWriteErrors()) {
    assert(false, "$setOnInsert _id.a - " + res.toString() + tojson(t.findOne()));
}

res = t.update( {"_id.a": 4} , { $setOnInsert: { "_id.b": 1 } } , true );
if (!res.hasWriteErrors()) {
    assert(false, "$setOnInsert _id.b - " + res.toString() + tojson(t.findOne()));
}

res = t.update( {"_id.a": 4} , { $setOnInsert: { "_id": {a:4, b:1} } } , true );
if (!res.hasWriteErrors()) {
    assert(false, "$setOnInsert _id 3 - " + res.toString() + tojson(t.findOne()));
}
